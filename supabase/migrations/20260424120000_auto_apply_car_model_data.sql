-- Auto-fyll: HP, drivlina, karosseri på Lovable från car_models-cache.
--
-- Bakgrund: enrich-batch hämtar bara 50 bilar/min och hann inte ifatt en
-- backlog på 33k bilar (skulle ta 13 dagar). Den här SQL-funktionen gör
-- samma jobb på millisekunder eftersom den bara läser ur car_models-
-- cachen — inga AI-anrop, ingen kostnad.
--
-- Schemaläggs var 5:e minut via pg_cron, så nya bilar som synkas från
-- Blocket får HP/drivlina/karosseri inom max 5 min utan att vänta på
-- enrich-batch-kön.
--
-- Skapades live via MCP apply_migration 2026-04-24; denna fil är till
-- för versionering i git.
...
BEGIN
  ...
  WITH up AS (
    UPDATE "Lovable" L
    SET horsepower = ROUND(((cm.typical_hp_min + COALESCE(cm.typical_hp_max, cm.typical_hp_min)) / 2.0))::int
    FROM public.car_models cm
    WHERE L.make = cm.make AND L.model = cm.model
      AND (L.horsepower IS NULL OR L.horsepower = 0)
      AND cm.typical_hp_min IS NOT NULL AND cm.typical_hp_min > 0
    RETURNING L.id
  )
...
-- Schemalägg: var 5:e minut. Idempotent — IF EXISTS-check via select.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'apply-cache-to-cars') THEN
    PERFORM cron.schedule(
      'apply-cache-to-cars',
      '*/5 * * * *',
      $cron$SELECT public.apply_car_model_data_from_cache();$cron$
    );
  END IF;
END $$;
